...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  '''y_hat = softmax(np.dot(outside_vectors, center_word_vec.T))      #(num words, 1)
 
  loss = -np.log(y_hat[outside_word_idx])
    
  y = y_hat.copy()
  y[outside_word_idx] -= 1
  grad_center_vec= np.dot(y, outside_vectors)
  grad_outside_vecs = np.dot(y[:, np.newaxis], center_word_vec[np.newaxis, :])'''
  prob_oc = softmax(outside_vectors.dot(center_word_vec))

  loss = -np.log(prob_oc[outside_word_idx])
  grad_center_vec = -outside_vectors[outside_word_idx,:] + outside_vectors.T.dot(prob_oc)
  grad_outside_vecs = np.array([ p*center_word_vec for p in prob_oc])
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  ### START CODE HERE
  '''center_word_indx = word2ind[current_center_word]
  centerWordVec = center_word_vectors[center_word_indx]

  for w in outside_words: 
    loss,_grad_center_vecs,_grad_outside_vectors = word2vec_loss_and_gradient(centerWordVec, word2ind[w], outside_vectors, dataset)
    loss += loss
    grad_center_vecs[center_word_indx] += _grad_center_vecs
  ...
  for word in outside_words:
    ...
    outside_id,outside_vectors,dataset)
    loss += loss_mini
        # print(gradCenterVecs[center_id].shape, gradCenter_mini.shape)
        # exit()
    grad_center_vecs[center_id] += gradCenter_mini
    grad_outside_vectors += gradOutside_mini
  ### END CODE HERE
